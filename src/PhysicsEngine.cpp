...
*                                                                           *
*****************************************************************************/
void physics::simulateStep(RigidBody* entity, int maxEntities){
	

	time+= DELTA_TIME;

	for (int body = 0; body< maxEntities; body++){
		entity->CMforce[DIM_Y] = G_ACCELLERATION * entity->CMmass;  // gravitation

		//		s0 = 0.5* (F/mass) * deltaT*deltaT + v0*deltaT + s0;
		moveMassPoint(&entity[body]);
		//      v0 = (F/mass)*deltaT+v0;
		entity[body].CMVeloVec[DIM_X] += entity->CMforce[DIM_X]/entity->CMmass * DELTA_TIME;   
		entity[body].CMVeloVec[DIM_Y] += entity->CMforce[DIM_Y]/entity->CMmass * DELTA_TIME;   
		entity[body].CMVeloVec[DIM_Z] += entity->CMforce[DIM_Z]/entity->CMmass * DELTA_TIME;

		spinMassPoint(&entity[body]);
		
		entity[body].calculateCenterOfMass();

		FLOAT_32 delta=0.1f;

		entity[body].bounce = getAndCorrectCollision(&entity[body]);

		if(entity[body].bounce != -1){
			calcCollisionResponse(&entity[body]);
		}

		if ((counter & entity[body].ANGLE_STEP) == 0){

			entity[body].move(counter);

			for (int t = 0; t<entity[body].maxJoints; t++){
				entity[body].joint[t].angle = entity[body].angle[t];
			}

			moveJoints(&entity[body]);
		}
	} // endfor body
	counter++;
